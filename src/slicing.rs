use crate::{
    tower::TowerVertex, Coord, Object, Settings, Slice,
    SlicerErrors, TriangleTower, TriangleTowerIterator,
};
use rayon::{
...
where V: Sync + Clone + TowerVertex + Debug {
    towers.into_par_iter().map(|tower| {
        let mut tower_iter = TriangleTowerIterator::new(tower);

        let mut layer = 0.0;

        // loop until max layers of printer
        let slices: Result<Vec<Slice>, SlicerErrors> =
            (0..(settings.print_z / settings.layer_height).round() as u32 + 1)
            .map(|layer_count| {
        ...
        // sort, as `par_bridge` isn't guaranteed to return in order
        s.par_sort_by(|a, b| {
            a.top_height
